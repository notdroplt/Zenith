...
void exec_instruction(register struct thread_t *thread) {
    ...
    switch (instruction.rtype.opcode) {
        ...
        case ld_byte_instrc:
            thread->registers[instruction.stype.rd] =
                fetch8(thread, thread->registers[instruction.stype.r1] + instruction.stype.immediate);
            break;
        case ld_half_instrc:
            thread->registers[instruction.stype.rd] =
                fetch16(thread, thread->registers[instruction.stype.r1] + instruction.stype.immediate);
            break;
        case ld_word_instrc:
            thread->registers[instruction.stype.rd] =
                fetch32(thread, thread->registers[instruction.stype.r1] + instruction.stype.immediate);
            break;
        case ld_dwrd_instrc:
            thread->registers[instruction.stype.rd] =
                fetch64(thread, thread->registers[instruction.stype.r1] + instruction.stype.immediate);
            break;
        /**/
        case st_byte_instrc:
        ...
        case auipc_instrc:
            ...
            break;
        case ecall_instrc:
            if (instruction.ltype.immediate) thread->halt_sig = 1;
            break;
        case ebreak_instrc:
            break;
        default:
...
int run(const char *filename, int argc, char **argv, void (*debugger)(struct thread_t *thread)) {
    ...
    if (debugger)
        ...
        while (!thread.halt_sig) exec_instruction(&thread);
