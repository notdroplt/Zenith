...
struct Node create_node(const enum NodeTypes type, const bool isconst) {
    return (struct Node){.type = type, .isconst = isconst};
}

struct Node *create_intnode(const uint64_t value) {
    struct NumberNode *node = malloc(sizeof(struct NumberNode));
    if (!node) return NULL;
    *node = (struct NumberNode){.base = create_node(Integer, true), .value = 0, .number = value};
...
struct Node *create_doublenode(const double value) {
    struct NumberNode *node = malloc(sizeof(struct NumberNode));
    if (!node) return NULL;
    *node = (struct NumberNode){.base = create_node(Double, true), .number = 0, .value = value};
...
struct Node *create_stringnode(struct string_t value, const enum NodeTypes type) {
    ...
    *node = (struct StringNode){.base = create_node(type, false), .value = value};
...
struct Node *create_unarynode(struct Node *value, const enum TokenTypes token) {
    ...
    *(struct UnaryNode *)node = (struct UnaryNode){.base = create_node(Unary, value->isconst), .token = token, .value = value};
...
struct Node *create_binarynode(struct Node *left, const enum TokenTypes token, struct Node *right) {
    ...
    *(struct BinaryNode *)node = (struct BinaryNode){
        .base = create_node(Binary, right->isconst && left->isconst), .left = left, .right = right, .token = token};
...
struct Node *create_ternarynode(struct Node *condition, struct Node *true_op, struct Node *false_op) {
    ...
    *(struct TernaryNode *)node = (struct TernaryNode){
        .base = create_node(Ternary, condition->isconst), .condition = condition, .trueop = true_op, .falseop = false_op};
...
struct Node *create_expressionnode(struct string_t name, struct Node *value) {
    ...
    if (!node) return NULL;
    *node = (struct ExpressionNode){.base = create_node(Expression, value->isconst), .name = name, .value = value};
...
struct Node *create_listnode(struct Array *node_array) {
    ...
    if (!node) return NULL;
    *node = (struct ListNode){.base = create_node(List, false), /* actually true but not now */
                              .nodes = node_array};
...
struct Node *create_switchnode(struct Node *expr, struct Array *cases) {
    ...
    if (!node) return NULL;
    *node = (struct SwitchNode){.base = create_node(Switch, expr->isconst), .expr = expr, .cases = cases};
...
struct Node *create_lambdanode(struct string_t name, struct Node *expression, struct Array *params_arr) {
    ...
    if (!node) return NULL;
    *node =
        (struct LambdaNode){.base = create_node(Lambda, false), .name = name, .expression = expression, .params = params_arr};
...
struct Node *create_includenode(struct string_t fname, bool binary) {
    ...
    if (!node) return NULL;
    *node = (struct IncludeNode){.base = create_node(Include, !binary), .fname = fname, .binary = binary};
...
struct Node *create_scopenode(struct Node *parent, struct Node *child) {
    ...
    if (!node) return NULL;
    *node = (struct ScopeNode){.base = create_node(Scope, parent->isconst), .parent = parent, .child = child};
...
struct Node *create_tasknode(struct Array *array) {
    ...
    if (!node) return NULL;
    *node = (struct TaskNode){.base = create_node(Task, false), .tasks = array};
...
static void delete_unarynode(struct UnaryNode *node) {
    if (node->value) delete_node(node->value);
    free(node);
}
...
static void delete_switchnode(struct SwitchNode *node) {
    for (size_t i = 0; i < array_size(node->cases); i++) {
        ...
        if (pair->first) delete_node(pair->first);
...
static void delete_typenode(struct TypeNode *node) {
    (void)node;
}
...
void delete_node(struct Node *node) {
    ...
    switch (node->type) {
        ...
        case Task:
            ...
            break;
        case Type:
            delete_typenode((struct TypeNode *)node);
            break;
        case Unused:
            break;
    }
...
int node_equals(const struct Node *left, const struct Node *right) {
    ...
    switch (left->type) {
        ...
        case String:
        case Identifier:
            if (typecast(struct StringNode *, left)->value.size != typecast(struct StringNode *, right)->value.size)
                return false;
            return strncmp(typecast(struct StringNode *, left)->value.string,
                           typecast(struct StringNode *, right)->value.string,
                           typecast(struct StringNode *, left)->value.size) == 0;
        case Unary:
            return typecast(struct UnaryNode *, left)->token == typecast(struct UnaryNode *, right)->token &&
                   node_equals(typecast(struct UnaryNode *, left)->value, typecast(struct UnaryNode *, right)->value);
        case Binary:
            return typecast(struct BinaryNode *, left)->token == typecast(struct BinaryNode *, right)->token &&
                   node_equals(typecast(struct BinaryNode *, left)->left, typecast(struct BinaryNode *, right)->left) &&
                   node_equals(typecast(struct BinaryNode *, left)->right, typecast(struct BinaryNode *, right)->right);
        case Ternary:
            return node_equals(typecast(struct TernaryNode *, left)->condition,
                               typecast(struct TernaryNode *, right)->condition) &&
                   node_equals(typecast(struct TernaryNode *, left)->trueop, typecast(struct TernaryNode *, right)->trueop) &&
                   node_equals(typecast(struct TernaryNode *, left)->falseop, typecast(struct TernaryNode *, right)->falseop);
        case Expression:
            if (typecast(struct ExpressionNode *, left)->name.size != typecast(struct ExpressionNode *, right)->name.size)
                return false;
            return strncmp(typecast(struct ExpressionNode *, left)->name.string,
                           typecast(struct ExpressionNode *, right)->name.string,
                           typecast(struct ExpressionNode *, left)->name.size) == 0 &&
                   node_equals(typecast(struct ExpressionNode *, left)->value, typecast(struct ExpressionNode *, right)->value);
        case List:
            if (array_size(typecast(struct ListNode *, left)->nodes) != array_size(typecast(struct ListNode *, right)->nodes))
                return false;
            return array_compare(typecast(struct ListNode *, left)->nodes, typecast(struct ListNode *, right)->nodes,
                                 (comparer_func)node_equals);
        case Call:
        case Index:
            return node_equals(typecast(struct CallNode *, left)->expr, typecast(struct CallNode *, right)->expr) &&
                   array_compare(typecast(struct CallNode *, left)->arguments, typecast(struct CallNode *, left)->arguments,
                                 (comparer_func)node_equals);
        case Lambda:
            if (typecast(struct LambdaNode *, left)->name.size != typecast(struct LambdaNode *, right)->name.size) return false;
            return strncmp(typecast(struct LambdaNode *, left)->name.string, typecast(struct LambdaNode *, right)->name.string,
                           typecast(struct LambdaNode *, left)->name.size) == 0 &&
                   node_equals(typecast(struct LambdaNode *, left)->expression,
                               typecast(struct LambdaNode *, left)->expression) &&
                   array_compare(typecast(struct LambdaNode *, left)->params, typecast(struct LambdaNode *, right)->params,
                                 (comparer_func)node_equals);
        case Switch:
            return node_equals(typecast(struct SwitchNode *, left)->expr, typecast(struct SwitchNode *, right)->expr) &&
                   array_compare(typecast(struct SwitchNode *, left)->cases, typecast(struct SwitchNode *, right)->cases,
                                 (comparer_func)node_equals);
        case Include:
            if (typecast(struct IncludeNode *, left)->fname.size != typecast(struct IncludeNode *, right)->fname.size)
                return false;
            return strncmp(typecast(struct IncludeNode *, left)->fname.string,
                           typecast(struct IncludeNode *, right)->fname.string,
                           typecast(struct IncludeNode *, left)->fname.size) == 0;
        case Scope:
            return node_equals(typecast(struct ScopeNode *, left)->parent, typecast(struct ScopeNode *, right)->parent) &&
                   node_equals(typecast(struct ScopeNode *, left)->child, typecast(struct ScopeNode *, right)->child);
        case Task:
            return array_compare(typecast(struct TaskNode *, left)->tasks, typecast(struct TaskNode *, right)->tasks,
                                 (comparer_func)node_equals);
            break;
        case Type:
            break;
...
void node_format(FILE *file, const struct Node *root) {
    switch (root->type) {
        ...
        case String:
        case Identifier:
            fprintf(file, "{\"type\":\"%s\",\"value\":%*s}", root->type == String ? "str" : "id",
                    (int)((struct StringNode *)root)->value.size, ((struct StringNode *)root)->value.string);
            break;
        case Unary:
        ...
        case Ternary:
            ...
            break;
        case Expression:
            fprintf(file, "{\"type\":\"expression\",\"name\":%*s,\"value\":", (int)((struct ExpressionNode *)root)->name.size,
                    ((struct ExpressionNode *)root)->name.string);
