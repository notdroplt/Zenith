...
static char lexNext(struct lex_t *lex) {
    if (lex->position.index >= lex->file_size) return '\0';
...
static struct token_t lexString(struct lex_t *lex) {
    ...
    struct token_t tok;
    lexNext(lex);
    while (lex->current_char != '"' && lex->current_char) lexNext(lex);
...
static double sstrtod(struct string_t str) {
    ...
    for (; str.string[i] != '.'; ++i) value = value * 10.0 + str.string[i] - '0';
...
static struct token_t lexIdentifier(struct lex_t *lex) {
    ...
    static const char *keywords[] = {"var",  "function", "as",     "do",      "switch", "default", "if", "then",
