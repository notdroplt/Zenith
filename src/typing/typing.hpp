...
namespace Typing
{
    ...
    public:
         ...
         *
         * But of course, not the entire Z set, only a computable subset
         *
        ...
        /**
         * @brief define when a value is a literal inside the code
         *
         * @return true for values that are literally written on code
         * @return false otherwise
         */
        virtual bool is_literal() const noexcept { return false; };

        /**
         * @brief define when a value is able to have and is signed
         *
         * @return true for values that are signed
         * @return false otherwise
         */
        virtual bool is_signed() const noexcept { return false; };

        /**
         * @brief define when a value is able to have but is not signed
         *
         * @return true for unsigned values
         * @return false otherwise
         */
        ...
        /**
         * @brief compare if a type is a base of other
         *
         * because this is a base class, all types will of course inherit from this one
         *
         * @return true if this is a base of other type
         * @return false otherwise
         */
        virtual bool is_base_of(const MetaType &) const noexcept { return true; }

        /**
         * @brief check if a type can be converted using casts or by being implictly castable
         *
         * @return true when the cast is allowed
         * @return false otherwise
         */
        virtual bool is_convertible_to(const MetaType &) const noexcept { return false; }

        /**
         * @brief check if a type can be converted only by using implict casts
         *
        ...
        /**
         * @brief operator overloaded to check if two types are equal
         *
        ...
        /**
         * @brief operator overloaded to check if current type is derived from another type
         *
         * @param type type to check inheritance
         * @return true if this inherits from other type
         * @return false otherwise
         */
        bool operator<(const MetaType &type) const noexcept { return this->is_base_of(type); }

        /**
         * @brief operator overloaded to check if current type is a base of another type
         *
         * @param type type to check inheritance
         * @return true when this is one of the bases of that type
         * @return false otherwise
         */
        bool operator>(const MetaType &type) const noexcept { return type.is_base_of(*this); }

        /**
         * @brief Cast a MetaType into any derived type
         *
         * @tparam T type to cast, needs to derive from MetaType
         * @return current value cast to a derived type
         */
        template <typename T>
            requires(std::is_base_of_v<MetaType, T>)
        constexpr T as()
        {
        ...
        /**
         * @brief Define operations to types
         *
         * Operating with types are the way the compiler knows how and where to optimize code
         *
         * @param operation type of operation to work with
         * @param operators operators used
         * @return returns<std::unique_ptr<MetaType>>
         */
        ...
        /**
         * @brief Destroy the Meta Type object
         *
         */
    ...
    class VoidType final : public MetaType
    {
        ...
        /**
         * @brief flag to check if type is void
         *
         * @return true for this type
         * @return false never for this type
         */
        virtual bool is_void() const noexcept override { return true; }

        /**
         * @brief returns the id of this type (-1)
         *
         * @return std::size_t
         */
    ...
    /**
     * @brief a type to define numerical ranges
     *
     */
    ...
    public:
        ...
        enum RangeSet
        {
            //! Represents integer numbers only
            Integers, 
         ...
         *
         * @return true when the superset is integers
         */
        ...
        /**
         * @brief overriding base class as ranges can be floating point
         *
         * @return true when the superset is real
         * @return false otherwise
         */
        ...
        /**
         * @brief generate a range that is either $$[0, border] \cap \mathbb{Z}\f$$ or $$[border, 0] \cap \mathbb{Z}$$
         * @tparam T any signed integer value
         */
