...
static const char *instructions[] = {
    ...
    "udiv", "udiv", "sdiv", "sdiv",
    "??", "??", "??", "??",
    "ld [byte]", "ld [half]", "ld [word]", "ld [dword]",
    "st [byte]", "st [half]", "st [word]", "st [dword]",
    "jal", "jal", "je", "jne",
    "jlu", "jls", "jleu", "jles",
    "setleu", "setleu", "setles", "setles",
    "setlu", "setlu", "setls", "setls",
...
void disassemble_instruction(union instruction_t inst)
{
    if (inst.ltype.opcode == 0x28 || inst.ltype.opcode > 0x37)
        ...
        printf("%s r%d, r%d, %ld\n", get_instruction(inst.ltype.opcode), inst.stype.rd, inst.stype.r1, (uint64_t)inst.stype.immediate);
}
