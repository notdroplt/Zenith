...
static return_t assemble_unary(struct Assembler *assembler, const struct UnaryNode *node)
{
    ...
    {
    case TT_Minus:
        inst = RInstruction(subr_instrc, used_reg, 0, used_reg);
        break;
    case TT_Not:
...
static return_t assemble_binary(struct Assembler *assembler, const struct BinaryNode *node, bool isJumping)
{
    ...
    return_t rchild = assemble(assembler, node->right);

    union instruction_t inst;

    if (rchild == -1LLU || lchild == -1LLU)
    {
        return lchild;
    }
...
static return_t assemble_identifier(struct Assembler *assembler, const struct StringNode *node)
{
    ...
    struct table_entry *function_table = NULL;
...
static return_t assemble_lambda(struct Assembler *assembler, const struct LambdaNode *node)
{
    ...
    if (!(arr = list_to_array(args)))
    {
        goto destructor_list;
    }

    if (!(entry = malloc(sizeof(struct table_entry))))
    {
        goto destructor_array;
    }
...
{
    uint64_t dot_v = 0;
    uint64_t used_true = 0;
    uint64_t status_true = 0;
    uint64_t used_false = 0;

    union instruction_t *last_instr = NULL;
...
void delete_table_entry(struct pair_t *entry_pair)
{
    struct table_entry *entry = entry_pair->second;
