...
void print_diff_status(register struct thread_t *old_thread, register struct thread_t *new_thread)
{
    ...
    printf("| inr: 0x%016lX -> 0x%016lX | memory size: %ld bytes | halt signal: %d |\n",
           old_thread->program_counter, new_thread->program_counter, new_thread->memory_size, new_thread->halt_sig);
    for (; i < 32; i += 4)
        printf("| r%02d:%s 0x%016lX " Color_Reset "->%s 0x%016lX " Color_Reset "| r%02d:%s 0x%016lX " Color_Reset "->%s 0x%016lX " Color_Reset "|\n"
...
void LIB_EXPORT debugger_func(struct thread_t *thread)
{
