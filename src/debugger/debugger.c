...
const char * color_old(uint64_t old, uint64_t new) {
    return old > new ? Color_Green : new > old ? Color_Red : "";
}

#define color_new(old, new) color_old(new, old)

void print_status(register struct thread_t *thread)
{
    register int i = 0;

    (void)fprintf(stdout, "| inr: 0x%016lX | memory size: %ld bytes | halt signal: %d |\n", 
...
void print_diff_status(register struct thread_t *old_thread, register struct thread_t *new_thread)
{
    ...
    for (; i < 32; i += 4)
    {
        ...
        fprintf(stderr, "| r%02d:%s 0x%016lX " Color_Reset "->%s 0x%016lX " Color_Reset "| r%02d:%s 0x%016lX " Color_Reset "->%s 0x%016lX " Color_Reset "|\n",
...
void debugger_func(struct thread_t *thread)
{
    ...
    while (1)
    {
        ...
        else if (strcmp(action, "continue") == 0)
        {
            ...
            (void)fprintf(stdout, "stopping on halt at:\n0x%08lX: ", thread->program_counter);
            disassemble_instruction(*(union instruction_t *)(thread->memory + thread->program_counter));
        }

        else if (strcmp(action, "instrc") == 0)
        {
            (void)fprintf(stdout, "0x%08lX: ", thread->program_counter);
            disassemble_instruction(*(union instruction_t *)(thread->memory + thread->program_counter));
        }
        ...
        {
            (void)fprintf(stdout, "\"%s\" is not recognized as a command.\n", action);
        }
