...
// operation
    ...
    const TContext = Context(*Types.Type);
    ...
    pub fn runAnalysis(self: *Analyzer, node: *Parser.Node) !*Parser.Node {
        return self.analyze(&self.context, node);
    }

    fn analyze(self: *Analyzer, currctx: *TContext, node: *Parser.Node) !*Parser.Node {
        switch (node.data) {
            .type => return node,
    ...
    fn analyzeInt(self: *Analyzer, node: *Parser.Node) Error!*Parser.Node {
        node.ntype = try self.talloc.create(Types.Type);
        node.ntype.?.* = Types.Type.initInt(node.data.int, node.data.int, node.data.int);
        return node;
    }

    fn analyzeDec(self: *Analyzer, node: *Parser.Node) Error!*Parser.Node {
        node.ntype = try self.talloc.create(Types.Type);
        node.ntype.?.* = Types.Type.initFloat(node.data.dec, node.data.dec, node.data.dec);
        return node;
    }

    fn analyzeStr(self: *Analyzer, node: *Parser.Node) Error!*Parser.Node {
        var arr = try self.talloc.alloc(?Types.Type, node.data.str.len);
        errdefer self.talloc.free(arr);

        const strType = try self.talloc.create(Types.Type);

        strType.* = Types.Type.initInt(0, 255, null);

        for (node.data.str, 0..) |c, i| {
            arr[i] = Types.Type.initInt(c, c, c);
        }

        var arrtype = try self.talloc.create(Types.Type);
    ...
    fn analyzeRef(self: *Analyzer, currctx: *TContext, node: *Parser.Node) Error!*Parser.Node {
    ...
    fn analyzeUnr(self: *Analyzer, currctx: *TContext, node: *Parser.Node) Error!*Parser.Node {
        node.ntype = try self.talloc.create(Types.Type);

        const rest = try analyze(self, currctx, node.data.unr.val);

        const unt = try self.talloc.create(Types.Type);

        unt.* = Types.Type.synthUnary(node.data.unr.op, rest.ntype.?.*) catch |err| {
    ...
    fn analyzeBin(self: *Analyzer, currctx: *TContext, node: *Parser.Node) Error!*Parser.Node {
        const lhs = try analyze(self, currctx, node.data.bin.lhs);
        const rhs = try analyze(self, currctx, node.data.bin.rhs);

        node.ntype = try self.talloc.create(Types.Type);

        node.ntype.?.* = Types.Type.binOperate(self.talloc, node.data.bin.op, lhs.ntype.?.*, rhs.ntype.?.*) catch |err| {
            self.errctx.position = node.position;
            return err;
        };
    ...
    fn analyzeCall(self: *Analyzer, currctx: *TContext, node: *Parser.Node) Error!*Parser.Node {
        const caller = try analyze(self, currctx, node.data.call.caller);
        const callee = try analyze(self, currctx, node.data.call.callee);

        node.ntype = try self.talloc.create(Types.Type);
        node.ntype = try Types.Type.callFunction(caller.ntype.?.*, callee.ntype.?.*);
        return node;
    }

    fn analyzeTernary(self: *Analyzer, currctx: *TContext, node: *Parser.Node) Error!*Parser.Node {
    ...
    fn analyzeIntr(self: *Analyzer, currctx: *TContext, node: *Parser.Node) AnalyzerError!*Parser.Node {
    ...
    fn analyzeRange(self: *Analyzer, currctx: *TContext, node: *Parser.Node) AnalyzerError!*Parser.Node {
        ...
        if (start.ntype.?.data == .integer and end.ntype.?.data == .integer and epsilon.ntype.?.data == .integer) {
            ...
            const trange = try self.talloc.create(Types.Type);
    ...
    fn analyzeModule(self: *Analyzer, currctx: *TContext, node: *Parser.Node) AnalyzerError!*Parser.Node {
    ...
    fn analyzeExpr(self: *Analyzer, currctx: *TContext, node: *Parser.Node) AnalyzerError!*Parser.Node {
        ...
        var freturn = try self.talloc.create(Types.Type);
        freturn.* = .{
            .pointerIndex = 0,
            .data = .{
        ...
        for (0..expr.params.len) |i| {
            ...
            const argT = try self.talloc.create(Types.Type);
            argT.* = .{
                .pointerIndex = 0,
                .data = .{
            ...
            const ft = try self.talloc.create(Types.Type);
        ...
        for (expr.params, 0..) |param, pidx|
            if (param.data == .ref) {
                ...
                var ptype: *Types.Type = undefined;
                if (node.ntype) |nt| {
                    ptype = try nt.getParameter(@intCast(pidx));
                } else {
                    ptype = try self.talloc.create(Types.Type);
                    ptype.* = Types.Type.initIdxCasting(self.castIndex);
                    self.castIndex += 1;
                }
